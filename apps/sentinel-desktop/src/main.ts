...
type Receipt = { payload: { state: "COMPLETED" | "FAILED"; processed_count: number; finding_count: number; findings: Finding[]; errors: AppError[]; rule_pack: { pack_id: string; expected_bytes_sha256: string }; yara?: { overall_status?: string; total_match_count?: number; packs?: Array<{ pack_id: string; scan_status: string; match_count: number; admitted_rule_count: number; quarantined_rule_count: number; corpus_content_id: string; freshness?: { state: string; age_days: number | null }; goodware_gate?: { status: string; match_count: number }; signing?: { status: string; signed: boolean | null; notarized: boolean | null; team_id?: string | null }; matches: Array<{ rule: string; source_pack_id?: string; metadata?: { description?: string | null; author?: string | null; reference?: string | null } }> }>; scan_status?: string; match_count?: number; matches?: Array<{ rule: string }>; rule_pack_content_id?: string } | null }; payload_sha256: string };
...
function renderResult(workspace: HTMLElement): void {
  ...
  if (receipt.payload.yara) { const y = receipt.payload.yara; const section = el("section"); section.append(el("h2", "YARA ENGINE — EXPERIMENTAL"), el("p", `Status  ${y.overall_status ?? y.scan_status ?? "UNKNOWN"}`), el("p", `Matches  ${y.total_match_count ?? y.match_count ?? 0}`)); for (const pack of y.packs ?? []) { const freshness = pack.freshness ? ` · ${pack.freshness.state}${pack.freshness.age_days === null ? "" : ` (${pack.freshness.age_days}d)`}` : ""; section.append(el("p", `${pack.pack_id} · ${pack.scan_status} · ${pack.match_count} matches · ${pack.admitted_rule_count} admitted · ${pack.quarantined_rule_count} quarantined${freshness}`)); if (pack.goodware_gate) section.append(el("p", `Goodware gate  ${pack.goodware_gate.status} · ${pack.goodware_gate.match_count} matches`)); if (pack.signing) section.append(el("p", `Signing  ${pack.signing.status}${pack.signing.team_id ? ` · Team ${pack.signing.team_id}` : ""}`)); for (const match of pack.matches) { section.append(el("p", `Rule  ${match.rule} · ${match.source_pack_id ?? pack.pack_id}`)); if (match.metadata?.description) section.append(el("p", `Description  ${match.metadata.description}`)); if (match.metadata?.author) section.append(el("p", `Author  ${match.metadata.author}`)); if (match.metadata?.reference) section.append(el("p", `Reference  ${match.metadata.reference}`)); } } for (const match of y.matches ?? []) section.append(el("p", `Rule  ${match.rule}`)); workspace.append(section); }
