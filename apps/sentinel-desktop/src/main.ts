...
function renderResult(workspace: HTMLElement): void {
  ...
  if (receipt.payload.yara) { const y = receipt.payload.yara; const section = el("section"); section.append(el("h2", "YARA ENGINE — EXPERIMENTAL"), el("p", `Status  ${y.overall_status ?? y.scan_status ?? "UNKNOWN"}`), el("p", `Matches  ${y.total_match_count ?? y.match_count ?? 0}`)); for (const pack of y.packs ?? []) { section.append(el("p", `${pack.pack_id} · ${pack.scan_status} · ${pack.match_count} matches · ${pack.admitted_rule_count} admitted · ${pack.quarantined_rule_count} quarantined`)); for (const match of pack.matches) section.append(el("p", `Rule  ${match.rule} · ${match.source_pack_id ?? pack.pack_id}`)); } for (const match of y.matches ?? []) section.append(el("p", `Rule  ${match.rule}`)); workspace.append(section); }
